...
class ElementTransformer {
	...
	def findPort(AsynchronousComponent component, Interface _interface, boolean conj) {
	...
	def getAllSubfunctions(ArchitectureSubcompnent subcompnent){
		val inst=trace.get(subcompnent) as AsynchronousComponentInstance
		val comp=inst.type as AsynchronousCompositeComponent
		val insts=comp.components
		return insts.filter[i|trace.contains(i)].map[i|trace.get(i) as ArchitectureSubfunction].toList
	}
