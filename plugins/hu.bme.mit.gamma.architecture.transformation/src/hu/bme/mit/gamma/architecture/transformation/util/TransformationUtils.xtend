...
class TransformationUtils {
	...
	static def getGammaName(Connector relation) {
		if (relation.name.isBlank){
			return relation.source.gammaName+"_to_"+relation.target.gammaName+"_Connector"
		}
		return relation.name.gammaName
	}
	
	static def getGammaName(ArchitectureRelation relation) {
		return relation.name.gammaName
	}
	...
	static def getSubsystems(ArchitecturePackage pkg){
		return pkg.architectureelement.filter[e | e instanceof System].map[e|e as System].filter[sys| ! ((!sys.relations.empty) && (!sys.subcompnents.empty)) ].toList
	}
