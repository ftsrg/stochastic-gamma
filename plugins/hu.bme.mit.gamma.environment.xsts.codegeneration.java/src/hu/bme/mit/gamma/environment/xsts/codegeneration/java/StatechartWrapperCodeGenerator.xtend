 ...
 ********************************************************************************/
package hu.bme.mit.gamma.environment.xsts.codegeneration.java
...
import static extension hu.bme.mit.gamma.expression.util.ExpressionEvaluator.*
...
class StatechartWrapperCodeGenerator {
	...
	final HashMap<TimeoutDeclaration,VariableDeclaration> timeoutMap

	new(String basePackageName, String statechartPackageName, StatechartDefinition gammaStatechart, XSTS xSts, HashMap<TimeoutDeclaration,VariableDeclaration> timeoutMap) {
	...
	protected def createStatechartWrapperClass() '''
		...
		public class «CLASS_NAME» implements «CLASS_NAME»Interface, TimedObject{
			// Port instances
			...
			public long getEarliestTime(){
				...
				«FOR timeout : gammaStatechart.timeoutDeclarations»
					...
					if ( («timeout.name»Time>=0) 
						...
						&& («timeout.name»RemainingTime<earliestTime) ) {
						earliestTime=«timeout.name»RemainingTime;
					}
				«ENDFOR»
				return earliestTime;
			}
