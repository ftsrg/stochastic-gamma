...
import hu.bme.mit.gamma.codegeneration.java.AsynchronousAdapterCodeGenerator
...
import static extension hu.bme.mit.gamma.codegeneration.java.Trace.*
...
«««				«ENDFOR»
			...
			private void init() {
				...
				«FOR match : QueuesOfClocks.Matcher.on(trace.getEngine).getAllMatches(component, null, null)»
					 timerService.setTimer(createTimerCallback(), «match.clock.name», «match.clock.timeSpecification.valueInMs», true);
				«ENDFOR»
			...
			private void performControlActions(«GAMMA_EVENT_CLASS» «EVENT_INSTANCE_NAME») {
				...
				«FOR controlSpecification : component.controlSpecifications»
					«IF controlSpecification.trigger instanceof AnyTrigger»
						...
						return;
					«ELSE»
						«FOR match : AnyPortTriggersOfWrappers.Matcher.on(trace.getEngine).getAllMatches(component, controlSpecification, null, null)»
							// Port trigger
						...
						«ENDFOR»
						«FOR match : PortEventTriggersOfWrappers.Matcher.on(trace.getEngine).getAllMatches(component, controlSpecification, null, null, null)»
							// Port event trigger
						...
						«ENDFOR»
						«FOR match : ClockTriggersOfWrappers.Matcher.on(trace.getEngine).getAllMatches(component, controlSpecification, null, null)»
							// Clock trigger
