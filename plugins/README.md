# Building Gamma and Stocjastic Gamma
...
2. Generate the Model plugin of the Environment Stochastic Gamma Models: `hu.bme.mit.gamma.environment.stochastic.model`. The Model plugin can be generated from the ecore file using a genmodel.
2. Generate the Model plugin of the Environment Composition Gamma Models: `hu.bme.mit.gamma.environment.model`. The Model plugin can be generated from the ecore file using a genmodel.
2. Generate the Model plugin of the Environment Analysis Gamma Models: `hu.bme.mit.gamma.environment.analysis.model`. The Model plugin can be generated from the ecore file using a genmodel.
7. Run `hu.bme.mit.gamma.environment.language/src/hu/bme/mit/gamma/environment/language/GenerateExpressionLanguage.mwe2` as a MWE2 Workflow.
If you encounter some buiding error try to delete the content of `src-gen` and `xtend-gen` folders and `clean` and `build` all projects.
