			...
			for (i : 0 .. row_nodes.length - 1) {
				...
				for (j : 0 .. fields.length - 1) {
					...
					if (field !== null) {
						...
						if (field.nodeName == "Classifier_guid") {
							classifierGUID = field.textContent
						}
	...
	static def getConnectorData(String xml) {
		...
		for (i : 0 .. row_nodes.length - 1) {
			...
			for (j : 0 .. fields.length - 1) {
				...
				if (field.nodeName == "Connector_ID") {
					connectorID = Integer.valueOf(field.textContent)
				}
				if (field.nodeName == "PDATA1") {
					PDATA1 = field.textContent
				}
			...
			if (sourceID == -1) {
				throw new MissingFormatArgumentException("Source of connection is not in XML: "+xml.replace("\\",""))
			}
			if (targetID == -1) {
				throw new MissingFormatArgumentException("Target of connection is not in XML: "+xml.replace("\\",""))
			}
			if (connectorID == -1) {
				throw new MissingFormatArgumentException("Connector ID is not in XML: "+xml.replace("\\",""))
			}
			if (GUID == "") {
				throw new MissingFormatArgumentException("GUID of connection is not in XML: "+xml)
			}
			dataList.add(new ConnectorData(name, sourceID, targetID, type, PDATA1, PDATA2, PDATA3,connectorID,GUID))
		}
		return dataList
	}
