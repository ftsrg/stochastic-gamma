...
class SQLUtils {
	...
	static def getAllProxyConnectors() '''
	select * from t_object where object_type='ProxyConnector';
	'''
	...
	static def getConnectorFlows()'''
	select * from t_xref where Name="MOFProps" and Type="connector property";
	'''
	...
	*
	...
	select 
		...
		t_connector.END_Object_ID as END_Object_ID,
		t_connector.Name as Name,
		t_connector.Connector_ID as Connector_ID,
		t_connector.ea_guid as ea_guid,
		t_xref.Description as Description
	from t_connector
