...
public class ExpressionParser {
	...
	public EventParameterReferenceExpression searchFlowProperty(String portName, String eventName)
		...
		var portIterator = StatechartModelDerivedFeatures.getAllPortsWithInput(statechart).stream()
				.filter(p -> p.getName().matches('^' + TransformationUtils.getGammaName(portName) +p.getInterfaceRealization().getInterface().getName() + "(In|Out)$"))
				.iterator();
		...
		if (portIterator.hasNext()) {
			throw new GammaTransformationException(
					"Parser Exception; Port reference is ambigous ('" + port.getName() + "' ; '"+portIterator.next().getName()+"') '" + portName + "' in '" + str + "'",statechart);
		}
