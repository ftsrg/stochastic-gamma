...
# time of model generation: 2024-05-09T21:43:39.033475600
...
class JointDistribution(TorchDistribution):
    ...
    def log_prob(self, value):
        log_probs=torch.zeros([self.dnum])
        for i in range(len(self.dists)):
            log_probs[i]=self.dists[i].log_prob(value[i]).sum()
        log_prob=log_probs.sum()
        #print(f"logprob: {log_prob} | value: {value}")
        return log_prob
...
class RandomVariable():
    ...
    def __init__(self,dist,name,N=20):
        self.dist=dist
        self.name=name
        self.event_cntr=-1
        self.N=N
...
class SampleImageBlur():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@1b490aad
		self.event_cntr=self.event_cntr+1
