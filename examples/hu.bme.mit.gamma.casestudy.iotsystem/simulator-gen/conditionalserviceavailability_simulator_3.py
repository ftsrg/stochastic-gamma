...
class RandomVariable():
	...
	def __init__(self,dist,name,N=60):
		self.dist=dist
		self.name=name
		self.event_cntr=N-1
		self.meta_cntr=-1
...
if47 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.EventStreamInterface$Listener$Provided')
if47_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if47,if47_s])
...
if48 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if48_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if48,if48_s])
...
if49 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if49_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if49,if49_s])
class SampleImageBlur():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@4521ebdc
		self.event_cntr=self.event_cntr+1
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.components["System.Camera1.softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer175")}},
		...
		self.components["System.Camera2.softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer176")}},
				portevents = 	{	"Events" : [ "NewEvent"	]},
				simulator=self)
		...
		self.components["System.Traffic.carArrival"].configure(
				...
				rules = {'Cars' : {'NewEvent' : RandomVariable(pyro.distributions.Exponential(torch.tensor(2.5)),"ContRandomVarriablecarArrival177")}},
		...
		self.components["System.Traffic.carDelay"].configure(
				...
				rules = {'CarOut' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.3),scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay178")}},
				simulator=self)
		...
		self.components["System.Camera1.Software.CameraSoftware.imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : RandomVariable(pyro.distributions.Bernoulli(self.failureProb[0]),"DiscRandomVarriableimageBlur181")}}},
				simulator=self,
				shname="imageBlur",
		...
		self.components["System.Camera2.Software.CameraSoftware.imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : RandomVariable(pyro.distributions.Bernoulli(self.failureProb[0]),"DiscRandomVarriableimageBlur182")}}},
				simulator=self,
				shname="imageBlur",
	...
	def reset(self):
		self.time=0
		self.events=[]#.clear()
		self.failureProb[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.01),high=torch.tensor(0.99)).mask(False)).detach()
		
		#for dist in self.dists:
		#	dist.reset()
		for i in pyro.plate("init_samples",len(self.dists)):
			self.dists[i].reset()
			#self.dists[i].samples=pyro.sample(self.dists[i].name+"_sample_"+str(self.dists[i].meta_cntr),self.dists[i].dist.expand([self.dists[i].N]))
			#0
		self.detmodel.reset()
	...
	def reset2(self):
		...
		beta_q = pyro.param(
			...
			torch.tensor(10.0), 
			constraint=constraints.positive)
		self.failureProb[0]=pyro.sample("param_0", 
