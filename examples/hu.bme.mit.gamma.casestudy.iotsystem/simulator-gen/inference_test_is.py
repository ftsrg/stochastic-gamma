from conditionalserviceavailability_2_simulator import *
import pyro
import math
import sys
import traceback
...
def cond_param_analysis(num_samples=1000):
    print(f"Run inference algorithm with {num_samples} samples")
    t0 = time.time()
    ...
    t1 = time.time()
    print(f"Analysis finished in {(t1-t0):.2f} s with ESS={ess:.2f}")
...
if __name__ == "__main__":
    sample_num=1000
    pyro.render_model(model,filename="test.png")
    if len(sys.argv)==2:
    ...
    try:
        cond_param_analysis(sample_num)
