...
class RV:
	plate=pyro.plate("random_var")

class ContinuousRandomVariable(RV):
	
	def __init__(self,dist,name,N=30):
		self.dist=dist
		self.name=name
		self.N=N
		#self.plate=pyro.plate("plate_"+name)
		self.event_cntr=-1
		#if self.N>1:
			#self.meta_cntr=0
			#with RV.plate:
				#self.samples=pyro.sample(self.name+"_sample_init",self.dist.expand([self.N]))
	...
	def calc(self,event=0,time=0):
		...
		if self.N>0:
			...
			with RV.plate:
				smpl = pyro.sample(self.name+"_sample_"+str(self.event_cntr),self.dist).item()
			return smpl
...
class SampleImageBlur():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@2beca739
		self.event_cntr=self.event_cntr+1
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		#self.failureProb=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.01),high=torch.tensor(0.99)))
		#0
		#0
		...
		self.components["System.Camera1.softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer40")}},
		...
		self.components["System.Camera2.softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer41")}},
				portevents = 	{	"Events" : [ "NewEvent"	]},
				simulator=self)
		...
		self.components["System.Traffic.carArrival"].configure(
				...
				rules = {'Cars' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(3.0)),"ContRandomVarriablecarArrival42")}},
		...
		self.components["System.Traffic.carDelay"].configure(
				...
				rules = {'CarOut' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.5),scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay43")}},
				simulator=self)
		...
		self.components["System.Camera1.networkLoss"].configure(
				...
				categorical=ContinuousRandomVariable(dist=pyro.distributions.Categorical(torch.tensor([
						0.9, 
										0.1
								])),
					name="NetworkLoss44")
		...
		self.components["System.Camera1.Software.CameraSoftware.imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(self.failureProb),"DiscRandomVarriableimageBlur46")}}},
				simulator=self,
				shname="imageBlur",
		...
		self.components["System.Camera2.Software.CameraSoftware.imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(self.failureProb),"DiscRandomVarriableimageBlur47")}}},
				simulator=self,
				shname="imageBlur",
	...
	def reset(self):
		...
		for dist in self.dists:
			dist.reset()
		##self.components[]
		failureProb=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.01),high=torch.tensor(0.99)))
		self.components["System.Camera1.Software.CameraSoftware.imageBlur"].rules={'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(failureProb),"DiscRandomVarriableimageBlur46")}}
		self.components["System.Camera2.Software.CameraSoftware.imageBlur"].rules={'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(failureProb),"DiscRandomVarriableimageBlur47")}}
		self.detmodel.reset()
