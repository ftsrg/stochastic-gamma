...
if26 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.EventStreamInterface$Listener$Provided')
if26_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if26,if26_s])
...
if27 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if27_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if27,if27_s])
...
if28 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if28_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if28,if28_s])
class SampleImageBlur():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@4e62ce06
		self.event_cntr=self.event_cntr+1
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.delayMean=torch.tensor([0.000001])
		#0
		#0
		...
		self.components[".System().Camera1()softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer92")}},
		...
		self.components[".System().Camera2()softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer93")}},
				portevents = 	{	"Events" : [ "NewEvent"	]},
				simulator=self)
		...
		self.components[".System().Traffic()carArrival"].configure(
				...
				rules = {'Cars' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(2.5)),"ContRandomVarriablecarArrival94")}},
		...
		self.components[".System().Traffic()carDelay"].configure(
				...
				rules = {'CarOut' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=self.delayMean[0],scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay95")}},
				simulator=self)
		...
		self.components[".System().Camera1().Software().CameraSoftware()imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.5)),"DiscRandomVarriableimageBlur98")}}},
				simulator=self,
				shname="imageBlur",
		...
		self.components[".System().Camera2().Software().CameraSoftware()imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.5)),"DiscRandomVarriableimageBlur99")}}},
				simulator=self,
				shname="imageBlur",
	...
	def reset(self):
		...
		for dist in self.dists:
			dist.reset()
		self.delayMean[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.1),high=torch.tensor(1.0))).detach()
		#0
		self.detmodel.reset()
		"""self.detmodel.reset(self.delayMean#1
		)"""
...
def simulate():
	...
	while len(stochmodel.events) > 0 and stochmodel.time < simTime:
		...
		if detmodel.monitorOfEndConditionSystemCarLeaveNewEvent.state != "run":
			# print debug end condition information
			if DEBUG:
				print("EndConditionSystemCarLeaveNewEvent : ", detmodel.monitorOfEndConditionSystemCarLeaveNewEvent.state)
			break
	
	
	#register the result of the analysis to the Pyro
	pyro.deterministic("Failures_newEvent_prob",torch.tensor(state2num(detmodel.monitorOfAspectSystemFailuresNewEvent.state)))
...
if __name__ == "__main__":
	try:
		...
		if DEBUG:
			...
			print("Estimated Failures_newEvent_prob = ",round(empirical_marginal_Failures_newEvent_prob.mean.item(),2))
