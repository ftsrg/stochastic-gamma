...
if23 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.EventStreamInterface$Listener$Provided')
if23_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if23,if23_s])
...
if24 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if24_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if24,if24_s])
...
if25 = JClass('hu.bme.mit.gamma.casestudy.iotsystem.interfaces.DataStreamInterface$Listener$Provided')
if25_s = JClass('hu.bme.mit.gamma.casestudy.iotsystem.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if25,if25_s])
class SampleImageBlur():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@3cd3a6ce
		self.event_cntr=self.event_cntr+1
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.components[".System().Camera1()softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer84")}},
		...
		self.components[".System().Camera2()softwareTimer"].configure(
				...
				rules = {'Events' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer85")}},
				portevents = 	{	"Events" : [ "NewEvent"	]},
				simulator=self)
		...
		self.components[".System().Traffic()carArrival"].configure(
				...
				rules = {'Cars' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(3.0)),"ContRandomVarriablecarArrival86")}},
		...
		self.components[".System().Traffic()carDelay"].configure(
				...
				rules = {'CarOut' : {'NewEvent' : ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.5),scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay87")}},
				simulator=self)
		...
		self.components[".System().Camera1().Software().CameraSoftware()imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.5)),"DiscRandomVarriableimageBlur90")}}},
				simulator=self,
				shname="imageBlur",
		...
		self.components[".System().Camera2().Software().CameraSoftware()imageBlur"].configure(
				...
				rules = {'ImageOut' : {'NewData' : {'blurred' : ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.5)),"DiscRandomVarriableimageBlur91")}}},
				simulator=self,
				shname="imageBlur",
