...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.components[".System()GPS1_Failure"].configure(
				...
				rules = {'Faults' : {'Failure' : ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS1_Failure6")}},
		...
		self.components[".System()GPS2_Failure"].configure(
				...
				rules = {'Faults' : {'Failure' : ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS2_Failure7")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
		...
		self.components[".System()Voter_Failure"].configure(
				...
				rules = {'Faults' : {'Failure' : ContinuousRandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure8")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
	...
	def reset(self):
		...
		for dist in self.dists:
			dist.reset()
		self.voterFailureRate[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.0),high=torch.tensor(10.0))).detach()
		#0
