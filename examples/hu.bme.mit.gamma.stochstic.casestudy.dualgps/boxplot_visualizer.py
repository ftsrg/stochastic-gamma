...
title1="GPS Simulation"
title2="GPS Importance Sampling"
y_label="Result of the analysis"
x_label="Number of simulations"
data=[]
with open('./dualgps_sim_conv_meas.csv', encoding='UTF8') as csv_file:
...
data2=[]
with open('./dualgps_is_conv_meas.csv', encoding='UTF8') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    for row in csv_reader:
        print(row)
        if len(row)>0:
