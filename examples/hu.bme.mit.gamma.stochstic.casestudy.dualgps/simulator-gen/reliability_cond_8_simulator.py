...
# time of model generation: 2024-05-11T10:47:34.847072800
...
def create_detmodel():
	...
	detmodel = 0
	EntryPoint = JClass('javaenv.Reliability_Cond_8EntryPoint')
	detmodel = EntryPoint()
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.components.update({ "System.GPS1Failure" : EventSource()})
		self.components.update({ "System.GPS2Failure" : EventSource()})
		self.components.update({ "System.GPS3Failure" : EventSource()})
		self.components.update({ "System.GPS4Failure" : EventSource()})
		self.components.update({ "System.GPS5Failure" : EventSource()})
		self.components.update({ "System.GPS6Failure" : EventSource()})
		self.components.update({ "System.GPS7Failure" : EventSource()})
		self.components.update({ "System.GPS8Failure" : EventSource()})
		...
		self.components["System.GPS1Failure"].configure(
				name  = "System.GPS1Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS1().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS1Failure0")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS2Failure"].configure(
				name  = "System.GPS2Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS2().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS2Failure1")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS3Failure"].configure(
				name  = "System.GPS3Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS3().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS3Failure2")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS4Failure"].configure(
				name  = "System.GPS4Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS4().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS4Failure3")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS5Failure"].configure(
				name  = "System.GPS5Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS5().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS5Failure4")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS6Failure"].configure(
				name  = "System.GPS6Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS6().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS6Failure5")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS7Failure"].configure(
				name  = "System.GPS7Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS7().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS7Failure6")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
				
		
		self.components["System.GPS8Failure"].configure(
				name  = "System.GPS8Failure",
				calls = {'Faults' : {'Failure' : [(lambda:self.detmodel.getSystem().getGPS8().getFaults().raiseFailure())]}},
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS8Failure7")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
		...
		self.components["System.Voter_Failure"].configure(
				...
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure8")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
...
def guide():
	...
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS1Failure0")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS2Failure1")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS3Failure2")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS4Failure3")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS5Failure4")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS6Failure5")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS7Failure6")]
	dists.append[RandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS8Failure7")]
	dists.append[RandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure8")]
