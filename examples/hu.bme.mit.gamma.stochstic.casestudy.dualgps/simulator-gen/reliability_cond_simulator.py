...
def create_detmodel():
	...
	detmodel = 0
	EntryPoint = JClass('javaenv.Reliability_CondEntryPoint')
	detmodel = EntryPoint()
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.components["System.Voter_Failure"].configure(
				...
				rules = {'Faults' : {'Failure' : RandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure2")}},
				portevents = 	{	"Faults" : [ "Failure"	]},
				simulator=self)
...
def guide():
	...
	dists.append[RandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure2")]
