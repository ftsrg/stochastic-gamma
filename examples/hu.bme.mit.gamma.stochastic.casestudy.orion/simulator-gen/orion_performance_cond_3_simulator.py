...
# time of model generation: 2024-05-10T22:22:17.681511600
...
def create_detmodel():
	...
	detmodel = 0
	EntryPoint = JClass('javaenv.Orion_Performance_Cond_3EntryPoint')
	detmodel = EntryPoint()
...
if148 = JClass('hu.bme.mit.gamma.stochastic.casestudy.orion.interfaces.StateMachine_Interface_For_OrionInterface$Listener$Provided')
if148_s = JClass('hu.bme.mit.gamma.stochastic.casestudy.orion.scheduling.ElementaryComponentSchedulingInterface')

@JImplements([if148,if148_s])
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.timerJitter=torch.tensor([0.000001])
		#0
		#0
		...
		self.components["Orion.TimerKeepAliveReceiveTimeout_3"].configure(
				...
				rules = {'TimoeutKeepAliveReceiveTimeout_3' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveReceiveTimeout_30")}},
		...
		self.components["Orion.TimerKapcsolodik_2"].configure(
				...
				rules = {'TimeoutKapcsolodik_2' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKapcsolodik_21")}},
		...
		self.components["Orion.TimerZarva_0"].configure(
				...
				rules = {'TimeoutZarva_0' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerZarva_02")}},
		...
		self.components["Orion.TimerKeepAliveSendTimeout_1"].configure(
				...
				rules = {'TimeoutKeepAliveSendTimeout_1' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveSendTimeout_13")}},
		...
		self.components["Orion.TimerKeepAliveReceiveTimeout_4"].configure(
				...
				rules = {'TimeoutKeepAliveReceiveTimeout_4' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveReceiveTimeout_44")}},
		...
		self.components["Orion.TimerKapcsolodik_3"].configure(
				...
				rules = {'TimeoutKapcsolodik_3' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKapcsolodik_35")}},
		...
		self.components["Orion.TimerKeepAliveSendTimeout_0"].configure(
				...
				rules = {'TimeoutKeepAliveSendTimeout_0' : {'NewEvent' : RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveSendTimeout_06")}},
	...
	def reset(self):
		self.time=0
		self.events.clear()
		self.timerJitter[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.1),high=torch.tensor(1.5))).detach()
		#0
		for i in pyro.plate("initial_samples",len(self.dists)):
			self.dists[i].reset()
		self.detmodel.reset()
		"""self.detmodel.reset(self.timerJitter#1
		)"""
...
def guide():
	...
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveReceiveTimeout_30")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKapcsolodik_21")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerZarva_02")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveSendTimeout_13")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveReceiveTimeout_44")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKapcsolodik_35")]
	dists.append[RandomVariable(pyro.distributions.Normal(loc=torch.tensor(5.0),scale=self.timerJitter[0]),"ContRandomVarriableTimerKeepAliveSendTimeout_06")]
...
def simulate():
	...
	pyro.sample("SystemConnStatus_conn_cond_ot",pyro.distributions.Normal(loc=torch.tensor(6.0),scale=torch.tensor(4.0)), obs = torch.tensor(stochmodel.time))
...
if __name__ == "__main__":
	try:
		...
		inference=pyro.infer.Importance(model=simulate, num_samples=5000)
		...
		empirical_marginal_SystemConnStatus_conn_mt2 = pyro.infer.EmpiricalMarginal(inference, "param_0")
		t1=time.time()
		# visualize results
		...
		print("Estimated param_0 = ",round(empirical_marginal_SystemConnStatus_conn_mt2.mean.item(),4))
