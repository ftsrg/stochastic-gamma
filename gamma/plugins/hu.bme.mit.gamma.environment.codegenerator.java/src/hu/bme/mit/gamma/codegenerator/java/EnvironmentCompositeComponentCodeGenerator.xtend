...
class EnvironmentCompositeComponentCodeGenerator extends SynchronousCompositeComponentCodeGenerator{
	...
	*/
	...
	protected def generateEnvironmentInports(AbstractSynchronousCompositeComponent component){
		'''
		«var comp=component as EnvironmentCompositeComponent»
		«var types=comp.environmentComponents
			.filter[c|c instanceof EnvironmentCompositeComponentInstance]
				.map[c|c as EnvironmentCompositeComponentInstance]
					.map[c|c.type]»
		«FOR t : types»
...
«««					Cascade components need their raised events saved (multiple schedule of a component in a single turn)
					...
					public «systemPort.name.toFirstUpper»() {
						...
						«FOR portBinding : systemPort.portBindings»
							«IF ! (portBinding.instancePortReference.instance instanceof EnvironmentComponentInstance)»
